...
Pipeline:
  1. ResNet-18 pretrained → extract feature map (layer1, layer2, layer3)
...
FEATURE_LAYERS  = ['layer1', 'layer2', 'layer3']   # ResNet-18 features
IMG_SIZE        = 256
BATCH_SIZE      = 16
TIMESTEPS_LIST  = [4, 8, 16, 32]

# Layer weights for fusion (can be adjusted)
LAYER_WEIGHTS = {
    'layer1': 0.2,  # Highest resolution, lowest semantic
    'layer2': 0.3,  # Middle resolution
    'layer3': 0.5,  # Lowest resolution, highest semantic
}


# ══════════════════════════════════════════════════════
# SECTION 1 — ResNet-18 Encoder (layer1, layer2, layer3)
# ══════════════════════════════════════════════════════

class ResNetEncoder(nn.Module):
    """
    ResNet-18 pretrained, bỏ avgpool và fc.
    Output: feature maps từ layer1, layer2, layer3
    - layer1: [B, 64, H/4, W/4]
...
# ══════════════════════════════════════════════════════
# SECTION 4 — Core scoring (with 3 layers)
# ══════════════════════════════════════════════════════

def compute_normal_stats(snn_encoder, normal_loader, device, timesteps):
    """
    Chạy SNN trên normal samples, thu thập mean firing rate cho 3 layers.
    """
    snn_encoder.eval()
    all_rates_f1 = []  # layer1 firing rates
    ...
    with torch.no_grad():
        for imgs, _ in normal_loader:
            ...
            for t in range(timesteps):
                f1, f2, f3 = snn_encoder(imgs)  # Now returns 3 outputs
                spike_acc_f1 = f1 if t == 0 else spike_acc_f1 + f1
                spike_acc_f2 = f2 if t == 0 else spike_acc_f2 + f2
                spike_acc_f3 = f3 if t == 0 else spike_acc_f3 + f3

            rate_f1 = (spike_acc_f1 / timesteps).mean(0)  # [C1, H1, W1]
            rate_f2 = (spike_acc_f2 / timesteps).mean(0)  # [C2, H2, W2]
            rate_f3 = (spike_acc_f3 / timesteps).mean(0)  # [C3, H3, W3]

            all_rates_f1.append(rate_f1.cpu())
            all_rates_f2.append(rate_f2.cpu())
            all_rates_f3.append(rate_f3.cpu())

    normal_rate_f1 = torch.stack(all_rates_f1).mean(0)  # [C1, H1, W1]
    normal_rate_f2 = torch.stack(all_rates_f2).mean(0)  # [C2, H2, W2]
    normal_rate_f3 = torch.stack(all_rates_f3).mean(0)  # [C3, H3, W3]
    
    return normal_rate_f1, normal_rate_f2, normal_rate_f3


def score_image(snn_encoder, img_tensor, normal_rate_f1, normal_rate_f2, normal_rate_f3,
                device, timesteps, scan_directions):
    """
    Tính anomaly score map cho 1 ảnh sử dụng cả 3 layers.
    """
    snn_encoder.eval()
    ...
    with torch.no_grad():
        for t in range(timesteps):
            f1, f2, f3 = snn_encoder(img_tensor)
            spike_acc_f1 = f1 if t == 0 else spike_acc_f1 + f1
            spike_acc_f2 = f2 if t == 0 else spike_acc_f2 + f2
            spike_acc_f3 = f3 if t == 0 else spike_acc_f3 + f3

    rate_f1 = spike_acc_f1 / timesteps  # [1, C1, H1, W1]
    rate_f2 = spike_acc_f2 / timesteps  # [1, C2, H2, W2]
    rate_f3 = spike_acc_f3 / timesteps  # [1, C3, H3, W3]

    # Deviation từ normal firing rate cho mỗi layer
    dev_f1 = (rate_f1[0] - normal_rate_f1.to(device)).abs().mean(0)  # [H1, W1]
    dev_f2 = (rate_f2[0] - normal_rate_f2.to(device)).abs().mean(0)  # [H2, W2]
    dev_f3 = (rate_f3[0] - normal_rate_f3.to(device)).abs().mean(0)  # [H3, W3]

    # Get target size (use layer2 as reference for upsampling)
    H2, W2 = dev_f2.shape
    
    # Upscale all layers to same size (H2, W2)
    dev_f1_up = F.interpolate(dev_f1.unsqueeze(0).unsqueeze(0),
    ...
    # Fuse 3 layers with weights
    score_spatial = (LAYER_WEIGHTS['layer1'] * dev_f1_up + 
                     LAYER_WEIGHTS['layer2'] * dev_f2 + 
                     LAYER_WEIGHTS['layer3'] * dev_f3_up)
    
    # Normalize score_spatial to [0, 1] range
    score_spatial = score_spatial / (score_spatial.max() + 1e-8)
...
def evaluate(snn_encoder, test_dataset, normal_rate_f1, normal_rate_f2, normal_rate_f3,
             device, timesteps, scan_directions):
    ...
    for i in range(len(test_dataset)):
        ...
        score_map, img_score = score_image(
            snn_encoder, img_t, normal_rate_f1, normal_rate_f2, normal_rate_f3,
            device, timesteps, scan_directions)
...
def main():
    ...
    # Verify activation range
    dummy = torch.randn(4, 3, IMG_SIZE, IMG_SIZE).to(device)
    with torch.no_grad():
        f1_d, f2_d, f3_d = ann_encoder(dummy)
    print(f"  layer1 max activation: {f1_d.max().item():.3f} (shape: {f1_d.shape})")
    print(f"  layer2 max activation: {f2_d.max().item():.3f} (shape: {f2_d.shape})")
    print(f"  layer3 max activation: {f3_d.max().item():.3f} (shape: {f3_d.shape})")
    ...
    normal_rate_f1, normal_rate_f2, normal_rate_f3 = compute_normal_stats(
        snn_encoder, normal_loader, device, timesteps=16)
    print(f"  Normal firing rate f1: mean={normal_rate_f1.mean():.4f}")
    ...
    for T in args.timesteps:
        nr_f1, nr_f2, nr_f3 = compute_normal_stats(
            snn_encoder, normal_loader, device, timesteps=T)

        img_auc, pix_auc = evaluate(
            snn_encoder, test_ds, nr_f1, nr_f2, nr_f3,
    ...
    with open(out_path, 'w') as f:
        ...
        f.write(f"Layer weights: {LAYER_WEIGHTS}\n\n")
