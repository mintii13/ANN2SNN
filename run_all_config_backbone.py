...
def print_plan(dataset, categories, configs):
    ...
    print(f"Estimated time: ~{total_runs * 10 / 60:.1f} hours (assuming 10 min/run)")
    print("=" * 70)
        ...
        if in_table and line and not line.startswith('='):
            ...
            if len(parts) >= 8:
                try:
                    ...
                    pass
            elif len(parts) >= 3:  # fallback cũ (2 cột)
                try:
                    t = int(parts[0].strip())
                    img_auc = float(parts[1].strip())
...
def create_summary_csv(config_save_dir, config_name, categories, timesteps):
    """Create a single summary CSV file containing all 7 metrics."""
    # Chuẩn bị 7 cột cho mỗi timestep: img_auc, img_ap, img_f1, pix_auc, pix_ap, pix_f1, pro
    data = {'Category': []}
    for T in timesteps:
        data[f'T{T}_img_auc'] = []
        data[f'T{T}_img_ap'] = []
        data[f'T{T}_img_f1'] = []
        data[f'T{T}_pix_auc'] = []
        data[f'T{T}_pix_ap'] = []   # thêm cột mới
        data[f'T{T}_pix_f1'] = []
        data[f'T{T}_pro'] = []
...
def create_comparison_table(base_save_dir_dataset, dataset_name, timesteps, categories):
    """Create a comparison table of all configs with mean of 7 metrics + mAD."""
    config_dirs = [d for d in os.listdir(base_save_dir_dataset) 
                   if os.path.isdir(os.path.join(base_save_dir_dataset, d))]
    results = []
    
    # Danh sách metric để tính mean (7 cột)
    metric_names = ['img_auc', 'img_ap', 'img_f1', 'pix_auc', 'pix_ap', 'pix_f1', 'pro']
    # Các metric dùng để tính mAD (bỏ pix_ap)
    mad_metrics = ['img_auc', 'img_ap', 'img_f1', 'pix_auc', 'pix_f1', 'pro']
            ...
            for T in timesteps:
                for metric in metric_names:
                    ...
                    if col in df.columns:
                        ...
                        mean_metrics[col] = 0
                # Tính mAD = mean của các metric trong danh sách mad_metrics
    ...
    # Tạo DataFrame: mỗi dòng là config, mỗi cột là T{}_metric
    comparison_data = []
    for res in results:
        row = {'Config': res['config'].replace(f'{dataset_name}_', '')}
        for T in timesteps:
            ...
            row[f'T{T}_mAD']     = round(res['metrics'].get(f'T{T}_mAD', 0), 4)   # cột mới
...
def main():
    ...
    print("  - comparison_table_{dataset}.csv (7 metrics + mAD)")
    print("=" * 70)
